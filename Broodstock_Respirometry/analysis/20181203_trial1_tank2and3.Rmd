---
title: "BroodstockRespirometryFall2018"
author: "Shelly Trigg"
date: "12/12/2018"
...
```{r, echo=FALSE}
ggplot(tank2, aes(interval_3sec,Value, color = Channel)) + geom_point() + geom_smooth(method='lm', se = FALSE) + facet_wrap(~Channel) + xlab("interval (3 seconds)") + ylab("umol O2/L") + ggtitle("O2 over time for tank2 respirometry trial 1, channel 4 = control")

ggplot(tank2, aes(interval_3sec,TransValue, color = Channel)) + geom_point() + xlab("interval (3 seconds)") + ylab("umol O2/L (scaled to intercept at 230umol/L)") + ggtitle("O2 over time for tank2 respirometry trial 1, channel 4 = control")
```

plot O2 over time for Tank 3  
```{r, echo=FALSE}
ggplot(tank3, aes(interval_3sec,Value, color = Channel)) + geom_point() + geom_smooth(method='lm', se = FALSE) + facet_wrap(~Channel) + xlab("interval (3 seconds)") + ylab("umol O2/L") + ggtitle("O2 over time for tank3 respirometry trial 1, channel 4 = control")

ggplot(tank3, aes(interval_3sec,TransValue, color = Channel)) + geom_point() + xlab("interval (3 seconds)") + ylab("umol O2/L (scaled to intercept at 230umol/L)") + ggtitle("O2 over time for tank3 respirometry trial 1, channel 4 = control")
...
```{r, echo = FALSE}
ggplot(tank2, aes(interval_3sec,Temp, color = Channel)) + geom_point() + geom_smooth(method='lm', se = FALSE) + facet_wrap(~Channel) + xlab("interval (3 seconds)") + ylab("temperature (C)") + ggtitle("temperature over time for tank 2 respirometry trial 1")
```


Plot Temperature over time for tank 3
```{r, echo = FALSE}
ggplot(tank3, aes(interval_3sec,Temp, color = Channel)) + geom_point() + geom_smooth(method='lm', se = FALSE) + facet_wrap(~Channel) + xlab("interval (3 seconds)") + ylab("temperature (C)") + ggtitle("temperature over time for tank 3 respirometry trial 1")
```

Tank2 and Tank3 temperature data on the same plot
```{r, echo = FALSE}
ggplot(tank2_3, aes(interval_3sec,Temp, color = tank)) + geom_point() + facet_wrap(~Channel) + xlab("interval (3 seconds)") + ylab("temperature (C)") + ggtitle("temperature over time for respirometry trial 1")
```

Tank2 and Tank3 O2 data on the same plot
```{r, echo=FALSE}
ggplot(tank2_3, aes(interval_3sec,TransValue)) + geom_point(aes(color = paste(tank,Channel))) + xlab("time interval (3 seconds)") + ylab("umol O2/L") + ggtitle("change in O2 over time;  different color for each tank.channel combo")

#plot with common colors for channels and tanks
ggplot(tank2_3, aes(interval_3sec,TransValue)) + geom_point(aes(color = tank)) + scale_color_manual(values = c("black", "magenta", "cyan", "darkgreen", "gold", "darkgrey","magenta", "cyan", "darkgreen", "gold")) + geom_line(aes(interval_3sec, TransValue,color = paste(tank,Channel))) + theme_bw() + xlab("time interval (3 seconds)") + ylab("umol O2/L (scaled to intercept at 230umol/L)") + ggtitle("change in O2 over time;  common colors for tanks and channels")
...
fit1 <- lm(Value ~ interval_3sec, data = tank2[which(tank2$Channel == "1"),])
fit2 <- lm(Value ~ interval_3sec, data = tank2[which(tank2$Channel == "2"),])
fit3 <- lm(Value ~ interval_3sec, data = tank2[which(tank2$Channel == "3"),])
fit4 <- lm(Value ~ interval_3sec, data = tank2[which(tank2$Channel == "4"),])
Tank2slopeIntercept <- data.frame(rbind(fit1$coefficients, fit2$coefficients, fit3$coefficients, fit4$coefficients))
colnames(Tank2slopeIntercept) <- c("Intercept (umol/L)", "slope (umol^-L/time (3 sec.))")
...
fit1 <- lm(Value ~ interval_3sec, data = tank3[which(tank3$Channel == "1"),])
fit2 <- lm(Value ~ interval_3sec, data = tank3[which(tank3$Channel == "2"),])
fit3 <- lm(Value ~ interval_3sec, data = tank3[which(tank3$Channel == "3"),])
fit4 <- lm(Value ~ interval_3sec, data = tank3[which(tank3$Channel == "4"),])
Tank3slopeIntercept <- data.frame(rbind(fit1$coefficients, fit2$coefficients, fit3$coefficients, fit4$coefficients))
colnames(Tank3slopeIntercept) <- c("Intercept (umol/L)", "slope (umol^-L/time (3 sec.))")
...
fit1 <- lm(TransValue ~ interval_3sec, data = tank2[which(tank2$Channel == "1"),])
fit2 <- lm(TransValue ~ interval_3sec, data = tank2[which(tank2$Channel == "2"),])
fit3 <- lm(TransValue ~ interval_3sec, data = tank2[which(tank2$Channel == "3"),])
fit4 <- lm(TransValue ~ interval_3sec, data = tank2[which(tank2$Channel == "4"),])
Tank2TRANSslopeIntercept <- data.frame(rbind(fit1$coefficients, fit2$coefficients, fit3$coefficients, fit4$coefficients))
colnames(Tank2TRANSslopeIntercept) <- c("Intercept (umol/L)", "slope (umol^-L/time (3 sec.))")
...
fit1 <- lm(TransValue ~ interval_3sec, data = tank3[which(tank3$Channel == "1"),])
fit2 <- lm(TransValue ~ interval_3sec, data = tank3[which(tank3$Channel == "2"),])
fit3 <- lm(TransValue ~ interval_3sec, data = tank3[which(tank3$Channel == "3"),])
fit4 <- lm(TransValue ~ interval_3sec, data = tank3[which(tank3$Channel == "4"),])
Tank3TRANSslopeIntercept <- data.frame(rbind(fit1$coefficients, fit2$coefficients, fit3$coefficients, fit4$coefficients))
colnames(Tank3TRANSslopeIntercept) <- c("Intercept (umol/L)", "slope (umol^-L/time (3 sec.))")
...
kable(tank2_3TRANSslopeIntercept, format = "markdown") %>% kable_styling(bootstrap_options = "striped", full_width = F)

```

```{r, echo = FALSE}
ggplot(tank2_3TRANSslopeIntercept[which(tank2_3TRANSslopeIntercept$Channel != "4"),c(2,6)]) + geom_boxplot(aes(Treatment,`slope (umol^-L/time (3 sec.))`, fill = Treatment)) + ggtitle("boxplots of change in O2 for channels 1-3 (controls not included)")
```