...
plot survival data
```{r}
ggplot(img_counts,aes(x = parental_currenttreatment, y = live.animals, color = parental_currenttreatment)) + geom_jitter(shape = 16, size = 3, position = position_jitter(0.2)) + theme_bw()

jpeg("SiloSurvCounts_vs_TreatmentGroup.jpeg", width = 8, height = 8, units = "in", res = 300)
ggplot(Oct18_counts,aes(x = parental_currenttreatment, y = avg.count, color = parental_currenttreatment)) + geom_jitter(shape = 16, size = 3, position = position_jitter(0.2)) + theme_bw() + ggtitle("End point survival") + ylab("Number animals alive") + xlab("parental.history_current.condition") + theme(legend.position = "none")
dev.off()
