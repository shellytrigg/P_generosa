...
#calculate days from date column
CALC.table$days <- as.Date(CALC.table$Date, format = "%m.%d.%y") - as.Date("09.04.19", format = "%m.%d.%y")
```
...
```{r}
jpeg("MetabolicRateOverTimeNormToWetWeight.jpg", width = 6,height = 3, units = "in", res = 300)
ggplot(CALC.table, aes(x = factor(days, levels = c(1,2,15,29,44)),y = resp.weight, group = interaction(Parental.history, Sw.Condition, days),fill = Sw.Condition, alpha = Parental.history)) + geom_boxplot(outlier.shape = NA) + geom_jitter(shape = 16, position = position_jitterdodge(jitter.width = 0.1)) + theme_bw() + labs(y = expression(paste("Respiration rate (ug ",O[2],  "/L*hr*gram)")), x ="days", fill = "juvenile pH exposure", alpha = "parental pH exposure") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_alpha_manual(values = c(1,0.4)) + scale_fill_manual(values = c("#00BFC4", "#F8766D"))
dev.off()
```
...
ggplot(CALC.table, aes(x = factor(days, levels = c(1,2,15,29,44)),y = Length, group = interaction(Parental.history, Sw.Condition, days),fill = Sw.Condition, alpha = Parental.history)) + geom_boxplot(outlier.shape = NA) + geom_jitter(shape = 16, position = position_jitterdodge(jitter.width = 0.1)) + theme_bw() + labs(y = "shell length (um)", x ="days", fill = "juvenile pH exposure", alpha = "parental pH exposure") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_alpha_manual(values = c(1,0.4))
...
#run anova for non-normalized data
#for Sept. 5 trial
resp.wt.3way.aov <- anova_test(resp.weight ~ Sw.Condition*Parental.history*days, data = CALC.table[which(CALC.table$Date!="09.05.19"),])
...
decompose 3 way results
```{r}
#https://www.datanovia.com/en/lessons/anova-in-r/#three-way-independent-anova

#evaluate the effect of juvenile pH expsosure and parental exposure on metabolic rate at each time point
model  <- lm(resp.weight ~ days*Sw.Condition*Parental.history, data = CALC.table[which(CALC.table$Date!="09.05.19"),])

CALC.table[which(CALC.table$Date!="09.05.19"),] %>% group_by(days) %>% anova_test(resp.weight ~ Sw.Condition*Parental.history, error = model)

#calculate the simple simple main effect

treatment.effect <- CALC.table[which(CALC.table$Date!="09.05.19"),] %>%
  group_by(days, Parental.history) %>%
  anova_test(resp.weight ~ Sw.Condition, error = model)
treatment.effect
```

```{r}
ggplot(CALC.table, aes(x = factor(days, levels = c(1,2,15,29,44)),y = resp.weight, group = interaction(Parental.history, Sw.Condition, days),fill = Sw.Condition, alpha = Parental.history)) + geom_boxplot(outlier.shape = NA) + geom_jitter(shape = 16, position = position_jitterdodge(jitter.width = 0.1)) + theme_bw() + labs(y = expression(paste("Respiration rate (ug ",O[2],  "/L*hr*gram)")), x ="days", fill = "juvenile pH exposure", alpha = "parental pH exposure") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_alpha_manual(values = c(1,0.4)) + facet_wrap(~days, scales = "free")
...
```{r}
#run glm for 10-18
resp.glm <- glm(resp.weight ~ Parental.history*Sw.Condition*days, data = CALC.table[which(CALC.table$Date!="09.05.19"),])
summary(aov(resp.glm))
...
test_Wtmean <- summarise(test, WtAvg = mean(Wet.weight.g, na.rm = TRUE), WtSD = sd(Wet.weight.g, na.rm = TRUE), WtSE = sd(Wet.weight.g, na.rm = TRUE)/(sqrt(n())))
