...
histology_data <- read_xlsx("../../amb_v_constantlow_brood_data/histology_scoring/HistologyScores_STRIGG.xlsx")
```
...
ca_data_sum_corrected_info_stage <- merge(ca_data_sum_corrected_info,histology_data[,c("Geoduck.ID", "Stage")], by = "Geoduck.ID")


ggplot(ca_data_sum_corrected_info_stage[which(ca_data_sum_corrected_info_stage$Date == "20190123" |ca_data_sum_corrected_info_stage$Date == "20190221"  ),], aes(x = Stage, y = conc, group = interaction(Sex,Treatment,Stage),color = Treatment)) +  geom_point(size=1) +  theme_bw() + theme(axis.text.x = element_text(angle=90,hjust=0.95,vjust=0.2))  + labs(x = "Stage", y = "Calcium conc. (mg/dL)") + facet_wrap(~Sex + Date, scales = "free_x")
